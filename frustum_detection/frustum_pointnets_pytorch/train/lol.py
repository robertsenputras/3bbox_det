...
def test_one_epoch(model, loader):
    ...
    loss_calculator = FrustumPointNetLoss()
    test_losses = {
        'total_loss': 0.0,
    ...
    for i, data_dicts in tqdm(enumerate(loader), \
        ...
        with torch.no_grad():
            ...
            losses = loss_calculator(
                outputs['logits'], data_dicts_var['seg'],
                outputs['box3d_center'], data_dicts_var['box3d_center'], outputs['stage1_center'],
                outputs['heading_scores'], outputs['heading_residual_normalized'], outputs['heading_residual'],
                data_dicts_var['angle_class'], data_dicts_var['angle_residual'],
                outputs['size_scores'], outputs['size_residual_normalized'], outputs['size_residual'],
                data_dicts_var['size_class'], data_dicts_var['size_residual']
            )
            
            # Calculate metrics
            seg_correct = torch.argmax(outputs['logits'].detach().cpu(), 2).eq(data_dicts_var['seg'].detach().cpu()).numpy()
            seg_accuracy = np.sum(seg_correct) / float(data_dicts_var['point_cloud'].shape[-1])
            
            iou2ds, iou3ds = compute_box3d_iou(
                outputs['box3d_center'].detach().cpu().numpy(),
                outputs['heading_scores'].detach().cpu().numpy(),
                outputs['heading_residual'].detach().cpu().numpy(),
                outputs['size_scores'].detach().cpu().numpy(),
                outputs['size_residual'].detach().cpu().numpy(),
                data_dicts_var['box3d_center'].detach().cpu().numpy(),
                data_dicts_var['angle_class'].detach().cpu().numpy(),
                data_dicts_var['angle_residual'].detach().cpu().numpy(),
                data_dicts_var['size_class'].detach().cpu().numpy(),
                data_dicts_var['size_residual'].detach().cpu().numpy()
            )
            ...
            metrics = {
                ...
                'iou2d': iou2ds.mean(),
                'iou3d': iou3ds.mean(),
                'iou3d_0.7': np.sum(iou3ds >= 0.7)/data_dicts_var['point_cloud'].shape[0]
            }
...
def train():
    ...
    for epoch in range(MAX_EPOCH):
        ...
        train_acc /= n_batches*float(NUM_POINT)
        train_iou2d /= n_batches
        train_iou3d /= n_batches
        train_iou3d_70 /= n_batches

        if FLAGS.return_all_loss:
            train_mask_loss /= n_batches
            train_center_loss /= n_batches
            train_heading_class_loss /= n_batches
            train_size_class_loss /= n_batches
            train_heading_residual_normalized_loss /= n_batches
            train_size_residual_normalized_loss /= n_batches
            train_stage1_center_loss /= n_batches
            train_corners_loss /= n_batches
        ...
        if FLAGS.return_all_loss and not FLAGS.debug:
            writer.add_scalar('train_mask_loss',train_losses['mask_loss'])
            writer.add_scalar('train_center_loss',train_losses['stage1_center_loss'])
            writer.add_scalar('train_heading_class_loss',train_losses['heading_class_loss'])
            writer.add_scalar('train_size_class_loss',train_losses['size_class_loss'])
            writer.add_scalar('train_heading_residual_normalized_loss',train_losses['heading_residual_normalized_loss'])
            writer.add_scalar('train_size_residual_normalized_loss',train_losses['size_residual_normalized_loss'])
            writer.add_scalar('train_stage1_center_loss',train_losses['stage1_center_loss'])
            writer.add_scalar('train_corners_loss',train_losses['corners_loss'])

        if not FLAGS.debug:
            writer.add_scalar('test_total_loss',test_losses['total_loss'], epoch)
            writer.add_scalar('test_iou2d_loss',test_losses['iou2d'], epoch)
            writer.add_scalar('test_iou3d_loss',test_losses['iou3d'], epoch)
            writer.add_scalar('test_acc',test_metrics['seg_acc'], epoch)
            writer.add_scalar('test_iou3d_0.7',test_metrics['iou3d_0.7'], epoch)

        if FLAGS.return_all_loss:
            writer.add_scalar('test_mask_loss',test_losses['mask_loss'], epoch)
            writer.add_scalar('test_center_loss',test_losses['stage1_center_loss'], epoch)
            writer.add_scalar('test_heading_class_loss',test_losses['heading_class_loss'], epoch)
            writer.add_scalar('test_size_class_loss',test_losses['size_class_loss'], epoch)
            writer.add_scalar('test_heading_residual_normalized_loss',test_losses['heading_residual_normalized_loss'], epoch)
            writer.add_scalar('test_size_residual_normalized_loss',test_losses['size_residual_normalized_loss'], epoch)
            writer.add_scalar('test_stage1_center_loss',test_losses['stage1_center_loss'], epoch)
            writer.add_scalar('test_corners_loss',test_losses['corners_loss'], epoch)
        '''
