...
class FrustumPointNetv1(nn.Module):
    ...
    def forward(self, data_dicts):
        ...
        logits = self.InsSeg(point_cloud, one_hot)  # [bs, n_points, 2]

        # Get mask prediction
        mask_probs = torch.softmax(logits, dim=2)  # [bs, n_points, 2]
        mask = (mask_probs[:, :, 1] > 0.5).float()  # [bs, n_points]

        # Mask Point Centroid
        object_pts_xyz, mask_xyz_mean, mask = \
                 point_cloud_masking(point_cloud, mask)

        # T-Net
        object_pts_xyz = object_pts_xyz.cuda()
        center_delta = self.STN(object_pts_xyz, one_hot)
